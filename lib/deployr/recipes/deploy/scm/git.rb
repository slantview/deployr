...
require 'deployr/recipes/deploy/scm/base'
  ...
  module Deploy
    module SCM
      ...
      class Git < Base
        ...
        def check!
          raise Deployr::Error, "Command #{git} not found." unless File.which("#{git}")
        end
        ...
        def checkout(revision, destination)
          ...
          execute = []
          execute << "#{git} clone #{verbose} #{args.join(' ')} #{deployment.repository} #{destination}"
        ...
        def query_revision(revision)
          ...
          command = scm('ls-remote', deployment.repository, revision)
