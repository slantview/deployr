  ...
  module Deploy
    module SCM
      ...
      class Base
        ...
        def check!
          raise NotImplementedError, "SCM #{self.class.name} is not implemented."
        end
        ...
        private
          ...
          def variable(name, default = nil)
            if local? && deployment.exists?("local_#{name}".to_sym)
          ...
          # activity.
          def logger
            @logger ||= variable(:ui) || Deployr::UI.new(STDOUT, STDERR, STDIN, config)
          end
